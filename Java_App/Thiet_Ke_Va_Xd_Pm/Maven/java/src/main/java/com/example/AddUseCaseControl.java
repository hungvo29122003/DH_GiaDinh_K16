...
public class AddUseCaseControl implements InputBoundary {
    ...
    public void execute(RequestData requestData) {
        ...
        if(isThamDinh(strNumber1) && isThamDinh(strNumber2)){
            ...
            addEntity = new AddEntity(number1, number2);
            int result = addEntity.add();

            ResponseResult responseResult = new ResponseResult();
            responseResult.sumResult = result;
            responseResult.strNumber1 = requestData.strNumber1;
            responseResult.strNumber2 = requestData.strNumber2;
            addUIConsoleOutput.outResult(responseResult);
        }else{
            //khong hợp lệ
